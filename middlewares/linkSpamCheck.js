        ...
        if (this.validURL(msg)) {
            ...
            if (!(isExcluded && !this.validURL(msg))) {
                if (context['badges'] == null) {
                    ...
                    this.client.say(target, `${context['display-name']}, you're not allowed to send links! You can only submit links from ${excludedLinksStr} sites!`);
                    this.client.timeout(target, context['username'], this.timeout, 'Sharing a link'); // Purge
                    return false;
                }
