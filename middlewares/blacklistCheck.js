    ...
    runMiddleware(msg, context, target) {
        if (!modCheck(context)) {
            for (let index = 0; index < this.blacklist.length; index++) {
                ...
                if (!!element.test(msg)) {
                    this.client.say(target, `${context['display-name']}, It is forbidden to send a word to the blacklist!`);
                    this.client.timeout(target, context['username'], this.timeout, 'Blacklisted word'); // Purge
                    return false;
                }
            }
        }
